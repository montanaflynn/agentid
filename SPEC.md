...
**Policy**
An m-of-n threshold rule governing a Bot Record operation. A policy specifies a minimum number of controller signatures required before a given operation (such as a status update or controller change) may be executed. See Section 7.
...
where:
- `agentid'` is the hardened index `0x80000000` plus `0x61676964` (the ASCII encoding of `"agid"` interpreted as a 32-bit big-endian unsigned integer, value `1633771364`), yielding hardened index `0xE1676964`
- `agent_index'` is the hardened index `0x80000000` plus `agent_index`, where `agent_index` is a non-negative integer uniquely identifying the agent within the keystore
...
```json
{
  ...
  "proof": "eyJhbGciOiJFZERTQSIsImNydiI6IkVkMjU1MTkifQ.eyJpZCI6InVybjphZ2VudDpzaGEyNTY6YTNmMWMyZDRlNWY2YTdiOGM5ZDBlMWYyYTNiNGM1ZDZlN2Y4YTliMGMxZDJlM2Y0YTViNmM3ZDhlOWYwYTFiMiJ9.SIGNATURE"
}
```
...
| `proof` | string | REQUIRED (on mutated records) | Compact JWS signature as produced by Section 6.2. MUST be absent from the payload before signing (per Section 6.2 step 1). MUST be present in any Bot Record transmitted after a mutation. The initial creation record MUST also include a proof signed by the agent's own private key. |
...
4. Parse the canonicalized bytes as JSON to obtain the subject Bot Record. The subject Bot Record's `id` field identifies the agent being mutated.
5. Attempt self-signed verification first: retrieve the 32-byte Ed25519 public key from the subject Bot Record's `public_key` field and verify the Ed25519 signature over `BASE64URL(header) + "." + BASE64URL(payload)` per RFC 8032 Section 5.1.7. If verification succeeds, the mutation is accepted as self-signed.
6. If self-signed verification fails, attempt controller verification: iterate over the subject Bot Record's `controllers` list in order. For each controller Agent ID, retrieve the controller's Bot Record, extract its 32-byte Ed25519 public key from the `public_key` field, and attempt Ed25519 signature verification over `BASE64URL(header) + "." + BASE64URL(payload)` per RFC 8032 Section 5.1.7. If any listed controller's key successfully verifies the signature, the mutation is accepted as controller-signed. The controller whose key verifies the signature is the signing controller for purposes of Section 9 authority checks.
7. If neither self-signed nor controller-signed verification succeeds, reject the mutation.
